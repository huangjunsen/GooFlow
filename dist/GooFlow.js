/**
* 
* 不知道这个插件具体叫什么名字, 暂时命名为 GooFlow
* 2015-12-03 下载于 17素材： http://www.17sucai.com/pins/3175.html 的演示代码。
* 具体不知道版权归属, 也不知道出处, 更没有详细的文档, 所有东西通过源码来学习。
* 该源码的注释倒是挺详细, 基本都是原作者写的, 找了一下网上, 貌似在ITEYE上的这个人比较靠谱, 很像是作者：
* V0.6 http://www.iteye.com/topic/1130290
* V0.8 http://www.iteye.com/topic/1135092
*                                                                  —— By sam
* 更新记录：
* 2015-12-03--sam--1. 修复由于网页版下载下来之后产生的各种bug, 暂时命名为 V1.0 版本；
*                  2. 为 property 添加 workWidth 和 workHeight 变量, 用于控制画布大小；
*                  3. 合并 GooFunc 里面的 GooFunc.getElCoordinate 和 GooFunc.mousePosition 函数到此文件中, 如此, 便不需要多余的js文件了；
*                  4. 同步成min文件（Google压缩方式）。
* 2015-12-04--sam--1. 通过修改 loadData 内容, 实现加载数据的同时判断元素自增ID是否需要增加, 修复bug。（已同步）
*                  2. 增加 MainContener 元素容器, 产生（0,0）的绝对坐标, 修复多容器遮罩错位bug。（已同步）
* 2015-12-05--sam--1. 增加虚函数 onExpandClick 和其对应按钮 ico_expand , 这个内容需要修改css。
*                  2. 采用 font awesome 图标替换原来的位图图标, 大大减少图标的重新加载导致的问题, 这个内容需要修改css。
*                  3. 增加头部工具按钮的提示文字设置选项。
* 2015-12-08--sam--1. 修改块区的最小宽度和高度, 均为50。（已同步）
* 2015-12-27--sam--1. 增加setNodeType方法, 用于更改节点类型。（已同步）
* 2015-12-28--sam--1. 修复initDraw、initGroup中画布长宽的bug, 全面启用 workWidth 和 workHeight 变量控制画布大小。（已同步）
* 2016-01-09--sam--1. 修复onItemDel返回类型全部是node的bug。（已同步）
* 2016-04-18--sam--1. 将 getElCoordinate 和 mousePosition 合并到 GooFunc变量里面, 避免全局对象污染
...
function GooFlow(bgDiv,property){
    ...
    var height = property.height-2;    //2个px是border的大小
    var headHeight=0;
    // 设置容器大小
    ...
    if(property.haveHead){
        HtmlStr="<div class='GooFlow_head'><label title='"+property.initLabelText+"'>"+property.initLabelText+"</label><span></span>";
    ...
    if(property.haveTool){
        HtmlStr = "<div class='GooFlow_tool'"+(property.haveHead?"":" style='margin-top:3px'")+">"
            + "<div style='height:"+(height-headHeight-(property.haveHead? 7:10))+"px' class='GooFlow_tool_div'>"
            + "</div></div>";
        this.$bgDiv.append(HtmlStr);
        this.$tool=this.$bgDiv.find(".GooFlow_tool div");
        //未加代码：加入绘图工具按钮
        HtmlStr = "<a href='javascript:void(0)' id='"+this.$id+"_btn_cursor' "
            + (property.toolBtnRemarks['cursor']?"title='"+property.toolBtnRemarks['cursor']+"'":"")+">"
            + "<i class='fa ico_cursor'/></i>"
            + "<a href='javascript:void(0)' id='"+this.$id+"_btn_direct' "
            + (property.toolBtnRemarks['direct']?"title='"+property.toolBtnRemarks['direct']+"'":"")+">"
            + "<i class='fa ico_direct'/></i>";
        this.$tool.append(HtmlStr);
        if(property.toolBtns&&property.toolBtns.length>0){
            HtmlStr = "<span/>";
            for(var i=0;i<property.toolBtns.length;i++){//加入自定义按钮
                HtmlStr += "<a href='javascript:void(0)' id='"+this.$id+"_btn_"+property.toolBtns[i]+"' "
                    + (property.toolBtnRemarks[property.toolBtns[i]]?"title='"+property.toolBtnRemarks[property.toolBtns[i]]+"'":"")+">"
                    + "<i class='fa ico_"+property.toolBtns[i]+"'/></i></a>";
            }
            this.$tool.append(HtmlStr);
        }
        //加入分支和聚合结点,复合结点及区域划分框工具按钮
        if(property.haveGroup){
            HtmlStr = "<span/><a href='javascript:void(0)' id='"+this.$id+"_btn_group' "
                + (property.toolBtnRemarks['group']?"title='"+property.toolBtnRemarks['group']+"'":"")+">"
                + "<i class='fa ico_group'/></i>";
            this.$tool.append(HtmlStr);
        }
        toolWidth=31;
        this.$nowType="cursor";
        ...
        this.$editable = true;  //具有工具栏时, 一定是可编辑的。
    }
    ...
    this.$group=null;
    this.initGroup(width,height,workWidth,workHeight);
    if(this.$editable){
        this.$workArea.on("click",{inthis:this},function(e){
          ...
          var ev=GooFunc.mousePosition(e),
              t=GooFunc.getElCoordinate(this);
...
GooFlow.prototype={
    ...
    initDraw:function(id,width,height,workWidth,workHeight){
        ...
        $(this.$draw).delegate(tmpClk,"dblclick",{inthis:this},function(e){
            ...
            This.$textArea.val(oldTxt).css({display:"block",width:120,height:14,
                left:t.left-c.left+x-This.$workArea[0].parentNode.scrollLeft,
                top:t.top-c.top+y-This.$workArea[0].parentNode.scrollTop}).data("id",This.$focus).focus();
    ...
    initGroup:function(width,height,workWidth,workHeight){
        this.$group=$("<div class='GooFlow_work_group' style='width:"+workWidth+"px;height:"+workHeight+"px'></div>");//存放背景区域的容器
        ...
        this.$group.on("mousedown",{inthis:this},function(e){//绑定RESIZE功能以及移动功能
            ...
            This.$ghost.css({display:"block",
                ...
                top:This.$areaData[id].top+t.top-c.top-This.$workArea[0].parentNode.scrollTop+hack+"px",
                left:This.$areaData[id].left+t.left-c.left-This.$workArea[0].parentNode.scrollLeft+hack+"px",cursor:cursor});
            var X,Y;
            X=ev.x-t.left+This.$workArea[0].parentNode.scrollLeft;
            Y=ev.y-t.top+This.$workArea[0].parentNode.scrollTop;
            if(cursor!="move"){
            ...
            document.onmousemove=function(e){
                ...
                if(cursor!="move"){
                    ...
                    X=ev.x-vX-c.left;Y=ev.y-vY-c.top;
                    if(X<t.left-c.left-This.$workArea[0].parentNode.scrollLeft)
                        X=t.left-c.left-This.$workArea[0].parentNode.scrollLeft;
                    else if(X+This.$workArea[0].parentNode.scrollLeft+This.$areaData[id].width>t.left-c.left+This.$workArea.width())
                        X=t.left-c.left+This.$workArea.width()-This.$workArea[0].parentNode.scrollLeft-This.$areaData[id].width;
                    if(Y<t.top-c.top-This.$workArea[0].parentNode.scrollTop)
                        Y=t.top-c.top-This.$workArea[0].parentNode.scrollTop;
                    else if(Y+This.$workArea[0].parentNode.scrollTop+This.$areaData[id].height>t.top-c.top+This.$workArea.height())
                        Y=t.top-c.top+This.$workArea.height()-This.$workArea[0].parentNode.scrollTop-This.$areaData[id].height;
            ...
            document.onmouseup=function(e){
                ...
                if(cursor!="move")
                    ...
                    This.moveArea(id,X+This.$workArea[0].parentNode.scrollLeft-t.left+c.left,Y+This.$workArea[0].parentNode.scrollTop-t.top+c.top);
            }
        });
        //绑定修改文字说明功能
        this.$group.on("dblclick",{inthis:this},function(e){
            ...
            This.$textArea.val(oldTxt).css({display:"block",width:100,height:14,
                left:t.left-c.left+x-This.$workArea[0].parentNode.scrollLeft,
                top:t.top-c.top+y-This.$workArea[0].parentNode.scrollTop}).data("id",p.id).focus();
            This.$workArea.parent().one("mousedown",function(e){
                if(This.$textArea.css("display")=="block"){
                    This.setName(This.$textArea.data("id"),This.$textArea.val(),"area");
                    This.$textArea.val("").removeData("id").hide();
    ...
    switchToolBtn:function(type){
        this.$tool.children("#"+this.$id+"_btn_"+this.$nowType).removeClass('GooFlow_tool_btndown');
    ...
    addNode:function(id,json){
        ...
        if(json.type!="start"&&json.type!="end"){
            ...
            this.$nodeDom[id]=$("<div class='GooFlow_item"+mark+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico'><i class='fa ico_"+json.type+"'></i></td><td>"+json.name+"</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
            if(json.type=='complex')    this.$nodeDom[id].addClass('item_complex');
        }
        else{
            json.width=24;json.height=24;
            this.$nodeDom[id]=$("<div class='GooFlow_item item_round"+mark+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='0'><tr><td class='ico'><i class='fa ico_"+json.type+"'></i></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span'>"+json.name+"</div></div>");
        }
    ...
    initWorkForNode:function(){
        ...
        this.$workArea.delegate(".GooFlow_item > .span","dblclick",{inthis:this},function(e){
            ...
            This.$textArea.val(oldTxt).css({display:"block",height:$(this).height(),width:100,
                left:t.left-c.left+This.$nodeData[id].left-This.$workArea[0].parentNode.scrollLeft-24,
                top:t.top-c.top+This.$nodeData[id].top-This.$workArea[0].parentNode.scrollTop+26})
        ...
        this.$workArea.delegate(".ico + td","dblclick",{inthis:this},function(e){
            ...
            This.$textArea.val(oldTxt).css({display:"block",width:$(this).width()+24,height:$(this).height(),
                left:t.left-c.left+24+This.$nodeData[id].left-This.$workArea[0].parentNode.scrollLeft,
                top:t.top-c.top+2+This.$nodeData[id].top-This.$workArea[0].parentNode.scrollTop})
                .data("id",This.$focus).focus();
            This.$workArea.parent().one("mousedown",function(e){
                This.setName(This.$textArea.data("id"),This.$textArea.val(),"node");
                This.$textArea.val("").removeData("id").hide();
        ...
        this.$workArea.delegate(".GooFlow_item > div > div[class!=rs_close]","mousedown",{inthis:this},function(e){
            ...
            This.$ghost.css({display:"block",
                width:This.$nodeData[id].width-2+"px", height:This.$nodeData[id].height-2+"px",
                top:This.$nodeData[id].top+t.top-c.top-This.$workArea[0].parentNode.scrollTop+hack+"px",
                left:This.$nodeData[id].left+t.left-c.left-This.$workArea[0].parentNode.scrollLeft+hack+"px",cursor:cursor
            });
            var X,Y;
            X=ev.x-t.left+This.$workArea[0].parentNode.scrollLeft;
            Y=ev.y-t.top+This.$workArea[0].parentNode.scrollTop;
    ...
    addArea:function(id,json){
        ...
        +"<label>"+json.name+"</label><i class='fa' title='点击变换颜色'></i><div><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
...
$.fn.extend({
    createGooFlow: function (property) {
        var defaultOption = {
            ...
            toolBtns: ["start", "end", "fork", "join", "complex", "node", "task", "chat", "state", "plug"],
            toolBtnRemarks: {cursor:"选择指针", direct:"连线", start:"开始结点", end:"结束结点", 
                            task:"任务结点", node:"自动结点", chat:"决策结点", state:"状态结点", plug:"附加插件", 
                            fork:"分支结点", join:"联合结点", complex:"复合结点", group:"组织划分框编辑开关"},
            haveHead: true,      //是否显示头部工具栏
            headBtns: ["new", "open", "save", "undo", "redo", "reload", "expand"],
            headBtnRemarks: ['新建', '打开', '保存','撤销','重做','重置画布','扩大画布'],
            haveTool: true,      //是否显示左侧工具栏, 如果不显示, 则画布不可编辑
